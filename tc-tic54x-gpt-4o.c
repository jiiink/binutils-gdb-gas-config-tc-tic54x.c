...
static void tic54x_emit_char(char c) {
    expressionS expn = { .X_op = O_constant, .X_add_number = c };
    emit_expr(&expn, 2);
}

/* Walk backwards in the frag chain.  */

fragS *frag_prev(fragS *frag, segT seg) {
    segment_info_type *seginfo = seg_info(seg);
    if (!seginfo || !seginfo->frchainP) return NULL;
    
    fragS *fragp = seginfo->frchainP->frch_root;
    while (fragp && fragp->fr_next != frag) {
        fragp = fragp->fr_next;
    }
    return fragp ? fragp : NULL;
}

static fragS *bit_offset_frag(fragS *frag, segT seg) {
    while (frag) {
...
char *parse_expression(char *str, expressionS *expn) {
    char *original_input_line_pointer = input_line_pointer;
    input_line_pointer = str;
    expression(expn);
    char *parsed_position = input_line_pointer;
...
static void tic54x_asg(int x ATTRIBUTE_UNUSED) {
    ...
    if (quoted) {
        ...
        size_t len = input_line_pointer - start;
...
static void tic54x_eval(int x ATTRIBUTE_UNUSED) {
    ...
    char terminator = get_symbol_name(&name);
    if (!ISALPHA(*name)) {
        as_bad(_("symbols assigned with .eval must begin with a letter"));
        restore_line_pointer(terminator);
        ignore_rest_of_line();
        return;
    }
...
static void stag_add_field_symbols(struct stag *stag, const char *path, bfd_vma base_offset, symbolS *rootsym, const char *root_stag_name) {
    struct stag_field *field = stag->field;
    char *prefix = concat(path, *path ? "." : "", NULL);

    while (field) {
        char *name = concat(prefix, field->name, NULL);

        if (!rootsym) {
            symbolS *sym = symbol_new(
                name,
                absolute_section,
                &zero_address_frag,
                field->stag ? field->offset : base_offset + field->offset
            );
            SF_SET_LOCAL(sym);
...
static void stag_add_field(struct stag *parent, const char *name, bfd_vma offset, struct stag *stag) {
    ...
    struct stag_field **current_field = &(parent->field);
    while (*current_field != NULL) {
        current_field = &((*current_field)->next);
    }
    *current_field = sfield;
...
static void tic54x_struct(int arg) {
    ...
    if (!is_union) {
        SKIP_WHITESPACE();
        start_offset = is_end_of_stmt(*input_line_pointer) ? 0 : get_absolute_expression();
    }
...
static void tic54x_endstruct(int is_union) {
    if (!current_stag || current_stag->is_union != is_union) {
        as_bad(_(".end%s without preceding .%s"), is_union ? "union" : "struct", is_union ? "union" : "struct");
        ignore_rest_of_line();
        return;
    }
    ...
    int size = current_stag->is_union ? current_stag->size : abs_section_offset - S_GET_VALUE(current_stag->sym);
    ...
    struct stag *outer_stag = current_stag->outer;
    if (outer_stag != NULL) {
        stag_add_field(outer_stag, current_stag->name, S_GET_VALUE(current_stag->sym), current_stag);
    } else {
        subseg_set(stag_saved_seg, stag_saved_subseg);
    }

    current_stag = outer_stag;
}
...
static void tic54x_tag(int ignore ATTRIBUTE_UNUSED) {
    ...
    if (!stag) {
        as_bad(*name ? _("Unrecognized struct/union tag '%s'") : _(".tag requires a structure tag"), name);
        ignore_rest_of_line();
        return;
    }
    ...
    if (current_stag) {
        stag_add_field(current_stag, label, abs_section_offset - S_GET_VALUE(current_stag->sym), stag);
    } else {
        ...
        stag_add_field_symbols(stag, S_GET_NAME(sym), S_GET_VALUE(stag->sym), sym, stag->name);
    }

    free(label);

    if (current_stag && !current_stag->is_union) {
...
static void tic54x_struct_field(int type) {
    ...
    switch (type) {
        case 'b': case 'B': case 'c': case 'C': case 'h': case 'H':
        case 'i': case 'I': case 's': case 'S': case 'w': case 'W':
        case '*':
            size = 1;
            break;
    ...
    if (line_label == NULL) {
        ...
        stag_add_field(current_stag, fake, abs_section_offset - S_GET_VALUE(current_stag->sym), NULL);
    } else {
        char* label = xstrdup(S_GET_NAME(line_label));
        stag_add_field(current_stag, label, abs_section_offset - S_GET_VALUE(current_stag->sym), NULL);
        free(label);
    }
...
static void tic54x_cons(int type) {
    ...
    switch (type) {
        case 'l': 
        case 'L': 
        case 'x': octets = 4; break;
        case 'b': 
        case 'B': 
        case 'c': 
        case 'C': octets = 1; break;
    }
        ...
        if (*input_line_pointer == '"') {
            ...
            if (expn.X_op == O_constant) {
                ...
                if ((octets == 1 && (value > 0xFF || value < -0x100)) ||
                    (octets == 2 && (value > 0xFFFF || value < -0x10000))) {
                    as_warn(_("Overflow in expression, truncated to %d bits"), octets * 8);
                }
            }
...
void tic54x_global(int type) {
  ...
  while (1) {
    ...
    if (c == EOF) {
      break; // In case get_symbol_name fails
    }
...
static htab_t subsym_htab_create(void) {
    ...
    return htab_create_alloc(INITIAL_SIZE, hash_string_tuple, eq_string_tuple, free_subsym_ent, xcalloc, free);
}
...
static htab_t local_label_htab_create(void) {
    ...
    return htab_create_alloc(initial_size, hash_string_tuple, eq_string_tuple, free_local_label_ent, xcalloc, free);
}
...
static void tic54x_clear_local_labels(int ignored)
{
  if (local_label_hash != NULL && macro_level >= 0)
  {
    ...
    switch (arg) {
        ...
        default: {
            ...
            if (*input_line_pointer == '"') {
                ...
                name = concat(name, flags, NULL);
            } else {
                int c = get_symbol_name(&name);
                if (restore_line_pointer(c) != 0 || !demand_empty_rest_of_line()) {
                    free(name);
                    return;
                }
                name = concat(name, flags, NULL);
...
tic54x_space (int arg)
{
  ...
  if (octets <= 0)
    {
      as_warn(octets < 0 ? _(".space/.bes repeat count is negative, ignored") : _(".space/.bes repeat count is zero, ignored"));
      return;
    }
...
static void handle_tic54x_section_errors(const char *name, segT seg, int blocking_flag, flagword flags) {
    if (blocking_flag) {
...
static void restore_segment_state(segT current_seg, subsegT current_subseg) {
    subseg_set(current_seg, current_subseg);
    demand_empty_rest_of_line();
}

static void tic54x_usect(int x ATTRIBUTE_UNUSED) {
    char *name = NULL, *section_name = NULL;
    int size = 0, blocking_flag = 0, alignment_flag = 0;
    segT seg, current_seg = now_seg;
    ...
    if (get_symbol_name(&section_name) != ',') {
    ...
    if (*input_line_pointer == ',') {
        ++input_line_pointer;
        blocking_flag = (*input_line_pointer != ',') ? get_absolute_expression() : 0;
    ...
    seg_info(seg)->bss = 1;
    char *p = frag_var(rs_fill, 1, 1, 0, line_label, size * OCTETS_PER_BYTE, NULL);
    *p = 0;

    handle_tic54x_section_errors(name, seg, blocking_flag, flags);
    restore_segment_state(current_seg, current_subseg);
    free(name);
}
...
static enum cpu_version lookup_version(const char *ver) {
    ...
    if (strlen(ver) == 3 && strchr("123589", ver[2])) {
        return ver[2] - '0';
    }

    if (strlen(ver) == 5 && strchr("56", ver[2]) 
        && toupper(ver[3]) == 'L' && toupper(ver[4]) == 'P') {
        return ver[2] - '0' + 10;
    }
...
static void set_cpu(enum cpu_version version) {
    ...
    if (version == V545LP || version == V546LP) {
        ...
        if (symbolP != NULL) {
            SF_SET_LOCAL(symbolP);
            if (!symbol_table_insert(symbolP)) {
                // Handle error, if any specific error handling is required
            }
        }
        else {
            // Handle error when symbol_new returns NULL
        }
    }
}
...
static void tic54x_version(int x ATTRIBUTE_UNUSED) {
    ...
    *input_line_pointer = 0;
    ...
    } else if ((cpu != VNONE && cpu != version) || (assembly_begun && version != cpu)) {
        as_warn(_("CPU version conflict detected"));
    } else {
        set_cpu(version);
    }
...
{
  (void)arg; // Explicitly indicate unused parameter
  const char *error_message = "p2align not supported on this target";
  as_bad(error_message);
}
...
static void tic54x_field(int ignore ATTRIBUTE_UNUSED) {
    ...
    if (expn.X_op != O_constant) {
        ...
        if (value != (valueT)expn.X_add_number) as_warn(_("field value truncated"));
        value = expn.X_add_number;
        ...
        if (size > 0) {
            handle_bit_offset(size, value, &label);
        }
    }
    demand_empty_rest_of_line();
}

void handle_bit_offset(int size, valueT value, symbolS **label) {
    char *p;
    int bit_offset = frag_bit_offset(frag_now, now_seg);
    fragS *alloc_frag = bit_offset_frag(frag_now, now_seg);

    if (bit_offset == -1) {
        handle_bit_offset_unresolved(size, value);
    } else {
        if (bit_offset == 0 || bit_offset + size > 16) {
            ...
            p = get_value_position(alloc_frag);
            update_label_position(alloc_frag, label);
        }
        store_value_to_chars(p, size, value, alloc_frag);
    }
}

void handle_bit_offset_unresolved(int size, valueT value) {
    char *p;
    struct bit_info *bi = XNEW(struct bit_info);
    ...
    p = frag_var(rs_machine_dependent, 4, 1, 0, make_expr_symbol(&size_exp), 0, (char *)bi);
}

char *get_value_position(fragS *alloc_frag) {
    if (alloc_frag == frag_now) {
        return frag_now->fr_literal + frag_now_fix_octets() - 2;
    }
    return alloc_frag->fr_literal;
}

void update_label_position(fragS *alloc_frag, symbolS **label) {
    if (*label != NULL) {
        symbol_set_frag(*label, alloc_frag);
        if (alloc_frag == frag_now) S_SET_VALUE(*label, frag_now_fix() - 1);
        *label = NULL;
    }
}

void store_value_to_chars(char *p, int size, valueT value, fragS *alloc_frag) {
...
static int tic54x_initialized_section(segT seg) {
  segT info = seg_info(seg);
  if (info == NULL) {
    return 0; // Return an error value or handle NULL case
  }
  return (info->bss == 0);
}
...
static void tic54x_clink(int ignored ATTRIBUTE_UNUSED) {
    ...
    if (*input_line_pointer == '\"') {
        ...
        while (is_a_char((next_char = next_char_of_string()))) {
            // Empty loop body just to consume characters
        }

        if (next_char != '\"') {
            as_bad(_("Expected closing quote for section name"));
            ignore_rest_of_line();
            return;
        }
        ...
        if ((seg = bfd_get_section_by_name(stdoutput, name)) == NULL) {
    ...
    if (!demand_empty_rest_of_line()) {
        as_bad(_("Unexpected text after .clink directive"));
    }
}
...
static void tic54x_set_default_include(void) {
    ...
    const char *slash_pos = strrchr(curfile, '/');

    if (slash_pos != NULL) {
        size_t len = slash_pos - curfile;
        include_dir_maxlen = (len > include_dir_maxlen) ? len : include_dir_maxlen;
...
static void tic54x_include(int ignored) {
    ...
    if (*input_line_pointer == '"') {
        ...
        int stmt_length = input_line_pointer - start;
        filename = strndup(start, stmt_length);
        if (!filename) {
    ...
    size_t input_length = strlen(filename) + strlen(newblock) + 4; // 4 for quotes and newline
...
static void tic54x_message(int type) {
    ...
    if (*input_line_pointer == '"') {
        ...
        msg_end = input_line_pointer;
        while (!is_end_of_stmt(*msg_end)) {
            msg_end++;
        }
        
        len = msg_end - input_line_pointer;
        ...
        strncpy(msg, input_line_pointer, len);
        msg[len] = '\0';

        input_line_pointer = msg_end;
    }
...
static void tic54x_label(int unused ATTRIBUTE_UNUSED) {
    char *name = NULL;
    symbolS *symbolP = NULL;
    int c = get_symbol_name(&name);

    if (c < 0 || !name) {
        fprintf(stderr, "Error getting symbol name\n");
        return;
    }

    symbolP = colon(name);
    if (!symbolP) {
        fprintf(stderr, "Error processing symbol\n");
        return;
    }
    ...
    if (!restore_line_pointer(c)) {
        fprintf(stderr, "Error restoring line pointer\n");
        return;
    }

    if (!demand_empty_rest_of_line()) {
        fprintf(stderr, "Error: line is not empty\n");
        return;
    }
}
...
static void tic54x_register_mmregs(int unusedParameter) {
    if (struct_is_illegal()) {
        return;
    }
...
{
  (void) ignore; // Suppress unused parameter warning
...
static void tic54x_sblock(int ignore ATTRIBUTE_UNUSED) {
    ...
    while (c == ',') {
        ...
        if (name == NULL) {
            as_bad(_("Failed to allocate section name"));
            ignore_rest_of_line();
            return;
        }

        seg = bfd_get_section_by_name(stdoutput, name);
        free(name); // free allocated name
...
static void tic54x_fclist(int show) {
    listing = show ? (listing & ~LISTING_NOCOND) : (listing | LISTING_NOCOND);
    demand_empty_rest_of_line();
}
...
static void tic54x_sslist(int show) {
    ...
    if (!isWithinStruct()) {
        ...
        // Handle illegal state appropriately
    }
}

static int isWithinStruct() {
    // This function should determine if the operation is within a struct
    // Replace with the actual logic for checking
...
static void tic54x_var(int ignore ATTRIBUTE_UNUSED) {
    ...
    while (true) {
        ...
        subsym_ent_t *ent = malloc(sizeof(*ent));
        if (!ent) {
            perror("malloc");
            exit(EXIT_FAILURE);
        }
...
static void tic54x_mlib(int ignore ATTRIBUTE_UNUSED) {
    ...
    file = search_and_open(filename, path);
    if (file) fclose(file);
    ...
    if (!abfd) {
        as_bad(_("can't open macro library file '%s' for reading: %s"), path, bfd_errmsg(bfd_get_error()));
        ignore_rest_of_line();
        return;
    }
    ...
    for (mbfd = bfd_openr_next_archived_file(abfd, NULL); mbfd != NULL; mbfd = bfd_openr_next_archived_file(abfd, mbfd)) {
        size = bfd_get_size(mbfd);
        buf = malloc(size);
        if (!buf) {
            as_bad(_("Memory allocation failed"));
            bfd_close(mbfd);
        ...
        fname = tmpnam(NULL);
        if (!fname) {
            as_bad(_("Failed to create temporary filename"));
            free(buf);
            bfd_close(mbfd);
            break;
        }
        ...
        if (!ftmp) {
            as_bad(_("Failed to open temporary file '%s': %s"), fname, strerror(errno));
            free(buf);
            bfd_close(mbfd);
            break;
        }
...
int md_parse_option(int c, const char *arg) {
    if (arg == NULL) {
        as_fatal(_("Argument cannot be NULL"));
        return 0;
    }
...
void tic54x_macro_info(const macro_entry *macro) {
    ...
    while (entry != NULL) {
        char *name = strndup(entry->name.ptr, entry->name.len);
        if (name == NULL) {
        ...
        subsym_ent_t *ent = malloc(sizeof(*ent));
        if (ent == NULL) {
            free(name); // Clean up in case of allocation failure
            return; // Handle memory allocation failure
        }

        ent->u.s = strndup(entry->actual.ptr, entry->actual.len);
        if (ent->u.s == NULL) {
            free(name);
            free(ent);
            return; // Handle memory allocation failure
...
void safe_htab_delete(void **hash_tab) {
    if (*hash_tab) {
        htab_delete(*hash_tab);
        *hash_tab = NULL;
    }
}
...
void tic54x_macro_end(void) {
    if (macro_level >= 0) {
        safe_htab_delete((void**)&subsym_hash[macro_level]);
        safe_htab_delete((void**)&local_label_hash[macro_level]);
        --macro_level;
    }
}
...
static int subsym_symlen(const char *a) {
    if (a == NULL) {
        return 0;
    }
    return (int)strlen(a);
...
static int subsym_symcmp(const char *a, const char *b) {
    if (!a || !b) {
        return -1; // Return a distinct value for NULL input
    }
    return strcmp(a, b);
}
...
static int subsym_firstch(const char *a, const char *b) {
    ...
    if (*endptr != '\0' || errno == ERANGE || val < 0 || val > CHAR_MAX) {
        return 0;
    }
...
static int subsym_lastch(const char *a, const char *b) {
    ...
    if (errno != 0 || *end != '\0' || val < 0 || val > 255) {
        return 0;
    }
...
static int subsym_isdefed(char *a) {
...
static bool subsym_ismember(const char *sym, const char *list) {
    if (!list) {
        return false;
    }
    ...
    if (!dup_list) {
        as_bad(_("Failed to duplicate list"));
        return false;
    }

    char *delim_pos = strchr(dup_list, ',');
    if (delim_pos) {
        *delim_pos = '\0';
        subsym_create_or_replace(sym, dup_list);
        subsym_create_or_replace(list, delim_pos + 1);
        free(dup_list);
        return true;
    }
...
static bool subsym_isname(const char *a, const char *ignore ATTRIBUTE_UNUSED) {
  if (!is_name_beginner(*a)) return false;
  for (const char *ptr = a; *ptr; ++ptr) {
    if (!is_part_of_name(*ptr)) return false;
  }
  return true;
}
...
static bool subsym_isreg(const char *a, const char *ignore ATTRIBUTE_UNUSED) {
  return str_hash_find(reg_hash, a) || str_hash_find(mmreg_hash, a);
}

/* Return the structure size, given the stag.  */

static int subsymStructSize(const char *name) {
    struct stag *stag = str_hash_find(stag_hash, name);
...
static float math_ceil(float arg1)
{
    return ceilf(arg1);
}

static int math_cvi(float arg1) {
    return (int)arg1;
}

#include <math.h>

float math_floor(float arg1) {
    return floorf(arg1);
}
...
static bool is_integer(float value) {
    int int_value = (int)value;
    return (value == (float)int_value);
}

#include <math.h>

static float math_round(float arg, float ignore ATTRIBUTE_UNUSED) {
    (void)ignore; // Explicitly ignore the unused parameter
    return roundf(arg);
}
...
{
    (void)ignore; // Explicitly mark ignore as unused
    return truncf(arg1); // Use standard library function for truncation
}

#include <math.h>
#include <errno.h>

static float math_acos(float arg)
{
    if (arg < -1.0f || arg > 1.0f) {
        errno = EDOM;
        return NAN;
    }
    return (float)acos(arg);
}

#include <math.h>
#include <errno.h>

static float math_asin(float arg1) {
...
static float math_atan(float arg1) {
...
static float math_atan2(float arg1, float arg2) {
    if (arg2 == 0.0f && arg1 == 0.0f) {
        errno = EDOM; 
        return NAN;
    }
...
static float math_cos(float arg1) {
    return cosf(arg1);
}

static float math_cvf(float arg1) {
    return arg1;
}

#include <math.h>

static float math_exp(float arg)
{
    return expf(arg);
}
...
static float safe_pow2(float exponent) {
    if (exponent > FLT_MAX_EXP - 1) {
        errno = ERANGE; 
...
static float math_ldexp(float arg1, float arg2) {
    float result = arg1 * safe_pow2(arg2);
    if (!isfinite(result)) {
        errno = ERANGE;
...
static float math_log10(float arg1) {
    if (arg1 <= 0.0f) {
        // Handle the error appropriately, e.g., by returning a specific error code or using errno
        return -1.0f; // example error code
    }
    return log10f(arg1);
}

static float math_log(float arg1) {
    if (arg1 <= 0.0f) {
        return NAN; 
    }
...
float math_min(float arg1, float arg2) {
...
static float math_pow(float base, float exponent) {
    if ((base == 0.0f && exponent <= 0.0f) || isnan(base) || isnan(exponent)) {
        errno = EDOM;
        return NAN;
    }
    ...
    if (fetestexcept(FE_INVALID)) {
        ...
        errno = ERANGE;
    } else if (fetestexcept(FE_OVERFLOW || fetestexcept(FE_UNDERFLOW))) {
        errno = ERANGE;
    }
...
static float math_sin(float arg1, float *result) {
    if (result == NULL) {
        errno = EINVAL;
        return -1.0f;
    }
...
static float math_sqrt(float arg1) {
    if (arg1 < 0.0f) {
        errno = EDOM;
        return -1.0f;
    }
...
static float math_tanh(float arg1) {
    return tanh(arg1);
}
...
void md_begin(void) {
    ...
    const char **symname;
    char *dir_env = getenv("TIC54X_DIR");
    if (!dir_env) {
        dir_env = getenv("A_DIR");
    }

    local_label_id = 0;

    if (dir_env) {
        char *dir_copy = notes_strdup(dir_env);
        char *tmp = dir_copy;
        char *next;
        
        while ((next = strchr(tmp, ';')) != NULL) {
            *next = '\0';
            add_include_dir(tmp);
            tmp = next + 1;
        }
...
void cleanup_hashes(void) {
    static htab *hashes[] = {
        stag_hash, subsym_recurse_hash, misc_symbol_hash, sbit_hash,
        cc3_hash, cc2_hash, cc_hash, mmreg_hash, reg_hash, parop_hash, op_hash
    };
    for (size_t i = 0; i < sizeof(hashes) / sizeof(hashes[0]); ++i) {
        htab_delete(hashes[i]);
    }
}

void end_macros(void) {
    while (macro_level != -1) {
        tic54x_macro_end();
    }
    macro_level = 0;
...
static int is_accumulator(struct opstruct *operand) {
    if (operand == NULL || operand->buf == NULL) return 0;
...
#define MAX_OPERANDS 10 // Example definition, adjust as necessary
...
static int get_operands(struct opstruct operands[], char *line) {
    ...
    while (numexp < MAX_OPERANDS && !is_end_of_stmt(*lptr)) {
        ...
        while (*lptr != ',' || paren_not_balanced) {
            if (*lptr == '\0') break;
            if (*lptr == '(') paren_not_balanced++;
            else if (*lptr == ')') paren_not_balanced--;
            lptr++;
        }
...
static int is_immediate(struct opstruct *operand) {
    if (operand == NULL || operand->buf == NULL) {
        return 0; // Treat null pointers as non-immediate
    }
    return *operand->buf == '#';
}
...
static int is_absolute(const struct opstruct *operand) {
    if (operand == NULL) {
        return 0; // Handle null pointer to avoid potential crashes
    }
...
static bool is_indirect(const struct opstruct *operand) {
    return operand != NULL && operand->buf[0] == '*';
}
...
static int is_dual(struct opstruct *operand) {
    ...
    if (strcasecmp(tmp, "-") == 0 || strcasecmp(tmp, "+") == 0 || strcasecmp(tmp, "+0%") == 0) {
        return 1;
    }
...
static int is_type(struct opstruct *operand, enum optype type) {
    ...
    switch (type) {
        ...
        case OP_xpmad: case OP_pmad: case OP_PA: case OP_dmad: 
        case OP_Lmem: case OP_MMR:
            return 1;

        case OP_Smem:
            return !is_immediate(operand);

        case OP_MMRY: case OP_MMRX:
            return is_mmreg(operand);

        case OP_SRC: case OP_SRC1: case OP_RND: case OP_DST:
            return is_accumulator(operand);

        case OP_B:
            return is_accumulator(operand) && toupper((unsigned char)operand->buf[0]) == 'B';

        case OP_A:
            return is_accumulator(operand) && toupper((unsigned char)operand->buf[0]) == 'A';

        case OP_ARX:
            return strncasecmp("ar", operand->buf, 2) == 0 && isdigit((unsigned char)operand->buf[2]);

        case OP_SBIT:
            return str_hash_find(sbit_hash, operand->buf) != NULL || is_absolute(operand);

        case OP_CC:
            return str_hash_find(cc_hash, operand->buf) != NULL;

        case OP_CC2:
            return str_hash_find(cc2_hash, operand->buf) != NULL;

        case OP_CC3:
            return str_hash_find(cc3_hash, operand->buf) != NULL || is_immediate(operand) || is_absolute(operand);
    ...
    switch (1) {
        case 1:
            if (insn->is_lkaddr) {
                ...
                handle_operand_expression(&insn->opcode[1], operand->exp);
            } else if (strncasecmp(operand->buf, "*sp (", 4) == 0) {
                return encode_address(insn, operand);
            } else {
                arf = (toupper(operand->buf[1]) == 'A') ? operand->buf[3] : operand->buf[4];
                arf -= '0';
                mod = determine_mod(insn, operand);
                if (mod < 0) {
                    as_bad("Unrecognized indirect address format \"%s\"", operand->buf);
                    return 0;
                }
            }
            break;
...
static void handle_operand_expression(opcode_struct *opcode, expr_struct expr) {
    if (expr.X_op == O_constant) {
        opcode->word = expr.X_add_number;
    } else {
        opcode->word = 0;
        opcode->r_nchars = 2;
        opcode->r_type = BFD_RELOC_TIC54X_16_OF_23;
        opcode->unresolved = 1;
    }
}

static int determine_mod(tic54x_insn *insn, struct opstruct *operand) {
    int mod = -1;
    if (operand->buf[1] == '+') {
        mod = 3;
        if (insn->tm->flags & FL_SMR) {
            as_warn("Address mode *+ARx is write-only. Results of reading are undefined.");
        }
...
int encode_integer(tic54x_insn *insn, struct opstruct *operand, int which, int min, int max, unsigned short mask) {
    long parse;
    long integer;
    ...
    if (operand->exp.X_op == O_constant) {
        ...
        integer = ((parse & 0x8000) && min == -32768 && max == 32767) ? (short)parse : parse;
...
static int encode_condition(tic54x_insn *insn, struct opstruct *operand) {
    ...
    const unsigned int CC_GROUP = 0x40;
    const unsigned int CC_ACC = 0x08;
    const unsigned int CATG_A1 = 0x07;
    const unsigned int CATG_B1 = 0x30;
    const unsigned int CATG_A2 = 0x30;
    const unsigned int CATG_B2 = 0x0C;
    const unsigned int CATG_C2 = 0x03;

    unsigned int insn_word = insn->opcode[0].word & 0xFF;
    if (insn_word != 0) {
        unsigned int insn_group = insn->opcode[0].word & CC_GROUP;
        unsigned int cc_group = cc->value & CC_GROUP;

        if (insn_group != cc_group) {
            as_bad(_("Condition \"%s\" does not match preceding group"), operand->buf);
            return 0;
        }

        unsigned int insn_acc = insn->opcode[0].word & CC_ACC;
        unsigned int cc_acc = cc->value & CC_ACC;
        if (insn_group) {
            if (insn_acc != cc_acc) {
                as_bad(_("Condition \"%s\" uses a different accumulator from a preceding condition"), operand->buf);
                return 0;
            }
            if ((insn_word & CATG_A1) && (cc->value & CATG_A1)) {
                as_bad(_("Only one comparison conditional allowed"));
                return 0;
            }
            if ((insn_word & CATG_B1) && (cc->value & CATG_B1)) {
                as_bad(_("Only one overflow conditional allowed"));
                return 0;
            }
        } else {
            if (((insn_word & CATG_A2) && (cc->value & CATG_A2)) ||
                ((insn_word & CATG_B2) && (cc->value & CATG_B2)) ||
                ((insn_word & CATG_C2) && (cc->value & CATG_C2))) {
...
static int encode_arx(tic54x_insn *insn, struct opstruct *operand) {
    if (strncmp("ar", operand->buf, 2) != 0 || strlen(operand->buf) < 3) {
...
int encode_cc2(tic54x_insn *insn, struct opstruct *operand) {
    if (!insn || !operand || !operand->buf) {
        as_bad("Invalid arguments");
        return 0;
    }

    tic54x_symbol *cc2 = str_hash_find(cc2_hash, operand->buf);
    if (!cc2) {
        as_bad("Unrecognized condition code \"%s\"", operand->buf);
        return 0;
    }
...
static int encode_operand(tic54x_insn *insn, enum optype type, struct opstruct *operand) {
    ...
    int index = ext ? (1 + insn->is_lkaddr) : 0;
    ...
    switch (type) {
        ...
        case OP_SRC:
            if (toupper(*operand->buf) == 'B') {
                insn->opcode[index].word |= (1 << 9);
                if (insn->using_default_dst)
                    insn->opcode[index].word |= (1 << 8);
            }
            return 1;

        case OP_RND:
            if (!((toupper(operand->buf[0]) == 'B') ^ ((insn->opcode[0].word & (1 << 8)) != 0))) {
        ...
        case OP_Xmem:
        case OP_Ymem: {
            int mod = (operand->buf[4] == '\0') ? 0 : (operand->buf[4] == '-') ? 1 : (operand->buf[5] == '\0') ? 2 : 3;
        ...
        case OP_MMR: {
            ...
            if (type == OP_MMR) {
                ...
                insn->opcode[0].word |= (value - 16) << (type == OP_MMRX ? 4 : 0);
            }
            return 1;
        }
        ...
        case OP_SHFT:
            return encode_integer(insn, operand, index, 0, 15, 0xF);

        case OP_SHIFT:
            return encode_integer(insn, operand, index, -16, 15, 0x1F);
        ...
        case OP_SBIT: {
            ...
            insn->opcode[0].word |= value | (reg << 9);
            return 1;
        }
        ...
        case OP_N:
            if (strcasecmp(operand->buf, "st0") == 0 || strcasecmp(operand->buf, "st1") == 0) {
                ...
                return 1;
            } else if (operand->exp.X_op == O_constant && (operand->exp.X_add_number == 0 || operand->exp.X_add_number == 1)) {
...
static void emit_insn(tic54x_insn *insn) {
    ...
    for (int i = 0; i < insn->words; i++) {
        ...
        if (!p) {
            as_warn(_("Memory allocation failed"));
            return;
        }

        valueT word_value = (size == 2) ? insn->opcode[i].word : (valueT) insn->opcode[i].word << 16;
        md_number_to_chars(p, word_value, size);
...
static int build_insn(tic54x_insn *insn) {
    if (!(insn->tm->flags & FL_PAR)) {
        for (int i = 0; i < insn->opcount; i++) {
            enum optype optype = OPTYPE(insn->operands[i].type);
            const char *buf = insn->operands[i].buf;
            if ((optype == OP_Smem || optype == OP_Lmem || optype == OP_Sind) &&
                strchr(buf, '(') && strncasecmp(buf, "*sp (", 4) != 0) {
                insn->is_lkaddr = 1;
                insn->lkoperand = i;
                break;
            }
...
static int optimize_insn(tic54x_insn *insn) {
    int adjusted_opcount = insn->opcount - 1;

    #define is_zero(op) ((op).exp.X_op == O_constant && (op).exp.X_add_number == 0)

    if (strcasecmp(insn->tm->name, "add") == 0 || strcasecmp(insn->tm->name, "sub") == 0) {
        if (insn->opcount > 1 
            && is_accumulator(&insn->operands[adjusted_opcount - 1]) 
            && is_accumulator(&insn->operands[adjusted_opcount]) 
            && strcasecmp(insn->operands[adjusted_opcount - 1].buf, insn->operands[adjusted_opcount].buf) == 0) {
            insn->opcount--;
            insn->using_default_dst = 1;
            return 1;
        }
        ...
        if (insn->opcount == 3 
            && ((OPTYPE(insn->tm->operand_types[0]) == OP_Smem && OPTYPE(insn->tm->operand_types[1]) == OP_SHIFT) 
            || (OPTYPE(insn->tm->operand_types[0]) == OP_Xmem && OPTYPE(insn->tm->operand_types[1]) == OP_SHFT)) 
        ...
        if (insn->opcount == 3 && insn->operands[0].type != OP_SRC 
            && (OPTYPE(insn->tm->operand_types[1]) == OP_SHIFT || OPTYPE(insn->tm->operand_types[1]) == OP_SHFT) 
            && is_zero(insn->operands[1]) 
            && (OPTYPE(insn->tm->operand_types[0]) != OP_lk || 
            (insn->operands[0].exp.X_op == O_constant && insn->operands[0].exp.X_add_number <= 255 && insn->operands[0].exp.X_add_number >= 0))) {
            insn->operands[1] = insn->operands[2];
            insn->opcount = 2;
            return 1;
        }
    } else if (strcasecmp(insn->tm->name, "sth") == 0 || strcasecmp(insn->tm->name, "stl") == 0) {
        if ((OPTYPE(insn->tm->operand_types[1]) == OP_SHIFT || OPTYPE(insn->tm->operand_types[1]) == OP_SHFT) && is_zero(insn->operands[1])) {
            insn->operands[1] = insn->operands[2];
            insn->opcount = 2;
            return 1;
        }
...
static int tic54x_parse_insn(tic54x_insn *insn, char *line) {
    ...
    int opcount = get_operands(insn->operands, line);
    if (opcount < 0) {
        return 0;
    }
    insn->opcount = opcount;

    while (insn->tm->name && strcasecmp(insn->tm->name, insn->mnemonic) == 0) {
        if (insn->opcount >= insn->tm->minops && insn->opcount <= insn->tm->maxops
            && operands_match(insn, insn->operands, insn->opcount,
                              insn->tm->operand_types, insn->tm->minops, insn->tm->maxops)) {
    ...
    as_bad(_("Unrecognized operand list '%s' for instruction '%s'"), line, insn->mnemonic);
    return 0;
}
...
static int tic54x_parse_parallel_insn_lastline(tic54x_insn *insn, char *line) {
        ...
        if (strcasecmp(insn->tm->parname, insn->parmnemonic) == 0) {
            if (insn->paropcount >= insn->tm->minops &&
                ...
                operands_match(insn, insn->paroperands, insn->paropcount,
                               insn->tm->paroperand_types, insn->tm->minops, insn->tm->maxops)) {
                return 1;
            }
        }
        ++(insn->tm);
    ...
    as_bad(valid_mnemonic ? _("Invalid operand (s) for parallel instruction \"%s\"") : _("Unrecognized parallel instruction combination \"%s || %s\""),
           valid_mnemonic ? insn->parmnemonic : insn->mnemonic, insn->parmnemonic);
    return 0;
}
...
static void subsym_create_or_replace(char *name, char *value) {
    ...
    if (!ent) {
        ...
        // Error handling code here (e.g., logging the error and returning)
        return;
    }
    ...
    int found = 0;
    for (int i = macro_level; i > 0 && !found; i--) {
...
static subsym_ent_t *subsym_lookup(char *name, int nest_level) {
    while (nest_level >= 0) {
...
int is_name_beginner(char c) {
    return isalpha(c) || c == '_';
}

int is_end_of_stmt(char c) {
    return c == '\0' || c == '\n';
}
...
char *concat(const char *s1, const char *s2, const char *s3, const char *s4) {
    char *result = xmalloc(strlen(s1) + strlen(s2) + strlen(s3) + strlen(s4) + 1);
    strcpy(result, s1);
    strcat(result, s2);
    strcat(result, s3);
    strcat(result, s4);
...
int get_symbol_name(char **name) {
    // Simulated function for getting a symbol name
    *name = "dummy";
    return 0;
}

int get_absolute_expression() {
    // Simulated function for getting an absolute expression
    return 1;
}

void *str_hash_find(void *hash, const char *key) {
    return NULL;
}

void str_hash_insert(void *hash, const char *key, void *value, int overwrite) {}

void str_hash_delete(void *hash, const char *key) {}

char *subsym_get_arg(const char *ptr, const char *delim, char **arg, int ismember) {
    // Simulated function for getting a substitution argument
    *arg = "arg";
    return (char *)ptr + strlen(ptr);
}

subsym_ent_t *subsym_lookup(const char *name, int macro_level) {
    return NULL;
}

char *subsym_substitute(char *line, int forced) {
    char *replacement = xstrdup(line);
    char *head = replacement;
    char *ptr = head;
    int changed = 0;
    int eval_line = 0;
    int eval_symbol = 0;
    ...
    while (!is_end_of_stmt(current_char = *ptr)) {
        if (eval_line) eval_end = strrchr(ptr, ',');
        ...
        if (eval_line && ptr >= eval_end) eval_symbol = 1;
        ...
        if ((forced && current_char == ':') || (!forced && is_name_beginner(current_char))) {
            ...
            if (str_hash_find(NULL, name) == NULL) {
                ent = subsym_lookup(name, 0);
                if (ent && !ent->isproc) value = ent->u.s;
            } else {
                as_warn("Substitution symbol recursion stopped at second appearance of '%s'", name);
            }
            ptr = tail = savedp;

            if ((*name == '$' && isdigit(name[1]) && name[2] == '\0') || name[strlen(name) - 1] == '?') {
                value = str_hash_find(NULL, name);
                if (!value) {
                    char digit[11];
                    ...
                    sprintf(digit, ".%d", 0);
                    strcat(value, digit);
                    str_hash_insert(NULL, namecopy, value, 0);
                }
                ptr = tail;
                ...
                *ptr = c;
                if (!ent->isproc) {
                    as_bad("Unrecognized substitution symbol function");
                    break;
                } else if (*ptr != '(') {
                    as_bad("Missing '(' after substitution symbol function");
                    break;
                }
                ++ptr;
                if (ent->ismath) {
                    ...
                    if (entry->nargs == 2) {
                        if (*ptr++ != ',') {
                            as_bad("Expecting second argument");
                            break;
                        }
                    ...
                    if (*ptr++ != ')') {
                        as_bad("Extra junk in function call, expecting ')'");
                        break;
                    }
                    recurse = 0;
                } else {
                    ...
                    if (entry->nargs == 2) {
                        if (*ptr++ != ',') {
                            as_bad("Function expects two arguments");
                            break;
                        }
                    ...
                    if ((!strcmp(entry->name, "$firstch") || !strcmp(entry->name, "$lastch")) && arg_type[1] != 2) {
                        as_bad("Expecting character constant argument");
                        break;
                    }
                    if (ismember && (arg_type[0] != 0 || arg_type[1] != 0)) {
                        as_bad("Both arguments must be substitution symbols");
                        break;
                    }
                    if (*ptr++ != ')') {
                        as_bad("Extra junk in function call, expecting ')'");
            ...
            if (value && !eval_symbol) {
                if (recurse) {
                    str_hash_insert(NULL, name, name, 0);
                    value = subsym_substitute(value, 0);
                    str_hash_delete(NULL, name);
                }
                ...
                *name = 0;
                if (forced && c == '(') {
                    ...
                    if (*tail++ != ')') {
                        as_bad("Missing ')' in subscripted substitution symbol expression");
                        break;
                    }
                    c = *tail;
                    input_line_pointer = savedp;
                ...
                char *tmp = xmalloc(strlen(head) + strlen(value) + strlen(tail + 1) + 2);
                strcpy(tmp, head);
                strcat(tmp, value);
                if (forced && c != ':') {
                    as_bad("Missing forced substitution terminator ':'");
                    break;
                }
                if (!forced) *tail = c;
                strcat(tmp, tail);
...
// Assume these functions, variables, and types are declared
extern char *input_line_pointer;
extern int macro_level;
extern int parallel_on_next_line_hint;
extern int substitution_line;
char *subsym_substitute(char *line, int force);
void input_scrub_insert_line(char *line);
int next_line_shows_parallel(const char *input);
int is_end_of_stmt(char c);
int is_whitespace(char c);
void *xmemdup0(const void *p, size_t sz);

void tic54x_start_line_hook(void) {
    char *line, *endp = input_line_pointer;
    ...
    char *replacement = (macro_level > 0) ? subsym_substitute(line, 1) : line;
    ...
    if (replacement != line) {
        ...
        while (is_whitespace(*tmp) && is_whitespace(*(tmp + 1)))
            ++tmp;
...
void md_assemble(char *line) {
    ...
    static bool is_parallel = false;
    static tic54x_insn insn;

    char *savedp = input_line_pointer;
    int c = get_symbol_name(&line);

    if (cpu == VNONE) cpu = V542;
    if (address_mode_needs_set) {
        set_address_mode(amode);
        address_mode_needs_set = false;
    }
    if (cpu_needs_set) {
        set_cpu(cpu);
        cpu_needs_set = false;
    ...
    *lptr = c;
    ...
    char *parallel_sym = strstr(line, "||");
    if (parallel_sym || parallel_on_next_line_hint) {
        if (parallel_sym) *parallel_sym = '\0';
        if (tic54x_parse_parallel_insn_firstline(&insn, lptr)) {
            is_parallel = true;
            if (parallel_sym) {
                parallel_sym += 2;
                while (is_whitespace(*parallel_sym)) ++parallel_sym;
                md_assemble(parallel_sym);
            }
        } else {
...
void tic54x_adjust_symtab(void) {
  if (!symbol_rootP || S_GET_STORAGE_CLASS(symbol_rootP) != C_FILE) {
    unsigned lineno;
    c_dot_file_symbol(as_where(&lineno));
  }
}
...
int tic54x_is_unrecognized_line(int c) {
    const int recognized_value = PARALLEL_SEPARATOR;
    return c == recognized_value;
}
...
symbolS *tic54x_undefined_symbol(char *name) {
    ...
    if (str_hash_find(cc_hash, name) || 
        (sym = str_hash_find(cc2_hash, name)) || 
        (sym = str_hash_find(cc3_hash, name)) || 
        str_hash_find(misc_symbol_hash, name) || 
        ...
        unsigned int value = sym ? sym->value : 0;
        return symbol_new(name, reg_section, &zero_address_frag, value);
    }
...
arelent *tc_gen_reloc(asection *section, fixS *fixP) {
    ...
    if (!sym) {
        as_fatal("Failed to retrieve symbol for relocation");
        return NULL;
    }

    rel = notes_alloc(sizeof(arelent));
    if (!rel) {
        as_fatal("Memory allocation for arelent failed");
        return NULL;
    }

    rel->sym_ptr_ptr = notes_alloc(sizeof(asymbol *));
    if (!rel->sym_ptr_ptr) {
        as_fatal("Memory allocation for symbol pointer failed");
        return NULL;
    }
    *rel->sym_ptr_ptr = sym;
    ...
    if (!rel->howto) {
        ...
        as_fatal("Cannot generate relocation type for symbol %s, code %s",
                 name ? name : "<unknown>", bfd_get_reloc_code_name(code));
        return NULL;
    }
...
void md_apply_fix(fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED) {
    ...
    switch (fixP->fx_r_type) {
        ...
        case BFD_RELOC_TIC54X_PARTLS7:
            val = (bfd_get_16(stdoutput, buf) & 0xFF80) | (val & 0x7F);
            bfd_put_16(stdoutput, val, buf);
            *valP = val & 0x7F;
            break;
        case BFD_RELOC_TIC54X_PARTMS9:
            val = (bfd_get_16(stdoutput, buf) & 0xFE00) | (val >> 7);
            bfd_put_16(stdoutput, val, buf);
            break;
        case BFD_RELOC_32:
        case BFD_RELOC_TIC54X_23:
            val = (bfd_get_32(stdoutput, buf) & 0xFF800000) | val;
...
valueT md_section_align(segT segment, valueT section_size) {
    (void)segment; // Suppresses unused parameter warning
    return section_size;
}
...
long md_pcrel_from(fixS *fixP)
{
  if (fixP == NULL)
  {
    return -1; // Indicate an error if fixP is NULL
  }
...
int tic54x_estimate_size_before_relax(fragS *frag, segT seg) {
    (void)frag; // Avoid unused parameter warning.
    (void)seg;
    return 0;
}
...
int tic54x_relax_frag(fragS *frag, long stretch ATTRIBUTE_UNUSED) {
    ...
    if (bi->type == TYPE_FIELD) {
        handle_field_type(bit_offset, available, size, bi, prev_frag, frag, &growth);
    } else {
        handle_other_types(bit_offset, available, size, bi, prev_frag, frag, &growth);
    }
...
void handle_field_type(int bit_offset, int available, int size, struct bit_info *bi, fragS *prev_frag, fragS *frag, int *growth) {
    if (bit_offset != 0 && available >= size) {
        merge_field_value(prev_frag, bi, available, size);
        ...
        *growth = -frag->fr_fix;
        ...
        apply_new_field_value(frag, bi, size);
        *growth = 0;
    }
}

void merge_field_value(fragS *prev_frag, struct bit_info *bi, int available, int size) {
...
void apply_new_field_value(fragS *frag, struct bit_info *bi, int size) {
...
void handle_other_types(int bit_offset, int available, int size, struct bit_info *bi, fragS *prev_frag, fragS *frag, int *growth) {
    if (bit_offset != 0 && bit_offset < 16) {
        if (available >= size) {
            update_prev_fragment_size(prev_frag, bi, size, frag, growth);
            return;
        }
    ...
    *growth = adjust_frag_data(frag, size);
}

void update_prev_fragment_size(fragS *prev_frag, struct bit_info *bi, int size, fragS *frag, int *growth) {
    if ((prev_frag->tc_frag_data += size) == 16) {
        prev_frag->tc_frag_data = 0;
    }
    if (bi->sym) {
...
int adjust_frag_data(fragS *frag, int size) {
    int growth = (size + 15) / 16 * OCTETS_PER_BYTE - frag->fr_fix;
...
void tic54x_convert_frag(bfd *abfd ATTRIBUTE_UNUSED, segT seg ATTRIBUTE_UNUSED, fragS *frag) {
    ...
    if (frag->fr_var == 0) {
        as_bad_where(frag->fr_file, frag->fr_line, _("division by zero in fragment conversion"));
        return;
    }
    frag->fr_offset = offset_bytes / frag->fr_var;

    if (frag->fr_offset < 0) {
        as_bad_where(frag->fr_file, frag->fr_line, _("attempt to .space/.bes backwards? (%ld)"), frag->fr_offset);
    }
...
int tic54x_start_label(char *label_start, int nul_char, int next_char) {
    ...
    // If within .struct/.union, no auto line labels.
    if (current_stag != NULL) {
        return 0;
    }

    // Disallow labels starting with "." if next char is not colon.
    ...
    // Do not define a label immediately followed by these directives.
